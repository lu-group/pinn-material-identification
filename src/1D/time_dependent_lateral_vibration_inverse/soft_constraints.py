...
def main():
    ...
    def pde(x, f):
        ...
        alpha = (tf.tanh(alpha_) + 1) * 2
        equilibrium = d2u_dt2 + alpha * d4u_dx4
    ...
    bc_l1 = dde.DirichletBC(geomtime, lambda x: 0, left_boundary)
    bc_r1 = dde.DirichletBC(geomtime, lambda x: 0, right_boundary)
    bc_l2 = dde.OperatorBC(
        geomtime, lambda x, y, _: dde.grad.hessian(y, x, i=0, j=0), left_boundary
    )
    bc_r2 = dde.OperatorBC(
        geomtime, lambda x, y, _: dde.grad.hessian(y, x, i=0, j=0), right_boundary
    )
    ic1 = dde.IC(
        geomtime, lambda x: np.sin(pi * x[:, 0:1]), lambda _, on_initial: on_initial
    )
    ic2 = dde.OperatorBC(
        geomtime,
    ...
    data = dde.data.TimePDE(
        geomtime,
        pde,
        [bc_l1, bc_r1, bc_l2, bc_r2, ic1, ic2, observe_u],
        num_domain=100,
        num_boundary=50,
        num_initial=50,
        num_test=50,
    ...
    model.compile("adam", lr=1e-3, external_trainable_variables=[alpha_])
    variable = dde.callbacks.VariableValue(
        [alpha_], period=1000, filename="lateral_variables.dat"
    )
